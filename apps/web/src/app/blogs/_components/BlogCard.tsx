...
export default function BlogCard({
  ...
  return (
    <Card
      className={cn(
        "group relative w-full overflow-hidden rounded-3xl border-2 border-gray-200 dark:border-gray-800 p-0 shadow-none backdrop-blur-sm transition-all duration-500",
        isGrid ? "h-full flex flex-col" : "h-full md:h-[320px]"
      )}
    >
      <div
        className={cn(
          "relative flex h-full flex-col",
          isGrid ? "w-full" : "md:flex-row"
        )}
      >
        {/* Left/Top: Thumbnail */}
        {post.coverImage && (
          <div
            className={cn(
              ...
              isGrid ? "h-48" : "h-56 md:h-full md:w-2/5"
            )}
          >
            <Image
              width={600}
        ...
        <CardContent
          className={cn(
            "flex h-full flex-col justify-between space-y-4 p-6 relative z-10",
            isGrid ? "flex-1" : "md:w-3/5 md:p-8"
          )}
        >
          <div className="space-y-4 overflow-hidden">
            ...
            <Link href={`/read/${post.slug}`} className="group/link block">
              <h3 className="text-xl md:text-2xl font-bold leading-tight bg-gradient-to-br from-foreground to-foreground/70 bg-clip-text text-transparent transition-all duration-300 group-hover/link:from-theme-primary group-hover/link:to-theme-secondary">
                {post.title}
              </h3>
            </Link>

            {/* Description */}
            <p className="line-clamp-3 text-sm md:text-base leading-relaxed text-muted-foreground">
              {post.descriptions}
            </p>
          </div>
